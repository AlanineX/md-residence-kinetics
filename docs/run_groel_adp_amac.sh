...
cp ../0216_GROEL_ADP/*.mdp ./
cp ../0216_GROEL_ADP/topol.top ./
cp ../0216_GROEL_ADP/test.gro ./
cp ../0216_GROEL_ADP/*.itp ./
cp ../0216_GROEL_ADP/*_Protein_chain_*.itp ./
...
# protein is 1um
# ADP is 100 um max in AmAc test, using equiv, we insert 100 ADP
# AmAc conc = 200 mM = [ 850/(6.02e23) ] / (19.14e-8)^3, so we insert 850 NH4
# for ACE, it is 200 mM (AMAC~1AC), which gives 850 ACE in the box
...
# NH4 = +1*850=+850
# ACE = -1*850=-850
# total = -1033, so we need 516.5 Mg to neutralize the system
# add 1 more NH4 to make it 516 Mg
gmx insert-molecules -f test_box.gro -ci ../solutes/ADP.pdb -radius 2 -o test_box_1.gro -nmol 300
gmx insert-molecules -f test_box_1.gro -ci ../solutes/NH4.pdb -radius 2 -o test_box_2.gro -nmol 851
gmx insert-molecules -f test_box_2.gro -ci ../solutes/ACE.pdb -radius 2 -o test_box_3.gro -nmol 850


# change the top
gmx solvate -cp test_box_3.gro -cs spc216.gro -o test_solvated.gro -p topol.top

# add water top path
# add salt ion top path

gmx grompp -f ions.mdp -c test_solvated.gro -p topol.top -o ions.tpr -maxwarn 0
gmx genion -s ions.tpr -o test_ions.gro -p topol.top -pname MG -pq 2 -np 516
